...
namespace SG.Business
{
    ...
    public class AttachmentStorage_SQL : IAttachmentStorage
    {
        ...
        public void DoPrepareDataSource()
        {
            ...
            //临时显示字段
            if (_attachmentStorage.Columns.IndexOf(tb_sys_AttachFile.IconLarge) < 0)
                _attachmentStorage.Columns.Add(tb_sys_AttachFile.IconLarge, typeof(byte[]));

            if (_attachmentStorage.Columns.IndexOf(tb_sys_AttachFile.IconSmall) < 0)
                _attachmentStorage.Columns.Add(tb_sys_AttachFile.IconSmall, typeof(byte[]));
            ...
            foreach (DataRow R in _attachmentStorage.Rows)
            {
                ...
                if (R[tb_sys_AttachFile.IconSmall] != DBNull.Value) continue;
        ...
        private byte[] GetFile(string fileName)
        {
            ...
            if (rows.Length > 0)
                return (byte[])rows[0][tb_sys_AttachFile.FFileBody];
            else
                return null;
        }
        /// <summary>
        ...
        {
            TAttachFile f = file as TAttachFile;
            DataRow[] rows = _attachmentStorage.Select("FID=" + f.FID.ToString());//查找旧记录
            if (rows.Length > 0)
            {
                DataRow row = rows[0];
                row[tb_sys_AttachFile.FDocID] = "";
        ...
        private void DoOpenFile(object attachmentDataRow)
        {
            try
            {
                ...
                byte[] file = (byte[])R[tb_sys_AttachFile.FFileBody]; //取文件内容
                string tempFile = CreateTempFile(file as byte[], R[tb_sys_AttachFile.FFileName].ToString());
    ...
    public class AttachmentOp
    {
        ...
        private IBridge_AttachFile CreateAttachFileBridge()
        {
            ...
            if (BridgeFactory.BridgeType == BridgeType.WebService)
                return new WcfIISHost_AttachFile();
            if (BridgeFactory.BridgeType == BridgeType.WcfHost)
                return new WcfHost_AttachFile();
