    ...
    public class bllBaseDataDict : bllBase, ILogSupportable
    {
        ...
        public virtual SaveResultEx UpdateEx(UpdateType updateType)
        {
            ...
            if (updateType == UpdateType.Add)
            {
                ...
                _DataBinder.Rows[0][_KeyFieldName] = bllComDataBaseTool.GetTableID(_SummaryTableName, _KeyFieldName);
            }
        ...
        /// 更新表的关联字段数据
        /// </summary>
        /// <param name="dtTable">表</param>
        /// <param name="sField">字段</param>
        /// <param name="sVal">更新值</param>
        protected virtual void UpdateTableFieldAllValue(DataTable dtTable, string sField, object sVal)
        {
        ...
        /// <param name="sField">字段</param>
        /// <param name="sVal">更新值</param>
        protected virtual void UpdateTableFieldValue(DataTable dtTable, string sField, int sVal)
        {
            foreach (DataRow row in dtTable.Rows)
            {
                if (row.RowState == DataRowState.Deleted) continue;
        ...
        /// <param name="sField">字段</param>
        /// <param name="sVal">更新值</param>
        /// <param name="bIsModifyWhenUpdate">当更新时是否修改值</param>
        protected virtual void UpdateTableFieldValue(DataTable dtTable, string sField, int sVal, bool bIsModifyWhenUpdate)
        {
            foreach (DataRow row in dtTable.Rows)
            {
                if (row.RowState == DataRowState.Deleted) continue;
                if (row.RowState == DataRowState.Added || (row.RowState == DataRowState.Modified && bIsModifyWhenUpdate))
                {
                    row[sField] = sVal;
                    sVal++;
                }
