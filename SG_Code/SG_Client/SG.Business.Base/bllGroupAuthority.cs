...
namespace SG.Business.Base
{
    ...
    public class bllGroupAuthority : bllBase
    {
        ...
        /// <summary>
        /// 用户组ID，默认为0，新增的时候获取，为其他表所用，非新增则将原有FID赋值
        /// </summary>
        private int _GroupID = 0;
        /// <summary>
        /// 权限表ID，如权限表FID为空，而_UGAuths=0，则获取FID，赋值以后_UGAuths+1，如果不为空，则不赋值
        /// </summary>
        private int _UGAuths = 0;
        /// <summary>
        /// 用户与组关系ID，如用户与组关系ID为空，而_UGID=0，则获取FID，赋值以后_UGID+1，如果不为空，则不赋值
        /// </summary>
        private int _UGID = 0;
        ...
        {
            DataRow[] rows = detailGroupAuths.Select(string.Format("FNUM='{0}' and FModelID={1}", menuName, moduleID));
        ...
        private void GetGroupAuthorityChangesChild(TreeNode node, DataTable auths)
        {
            foreach (TreeNode n in node.Nodes)
            {
                ...
                else if (tag.DataRow != null && n.Checked) //处理勾选状态的结点
                {
                    ...
                    if (changed != oringed) tag.DataRow[tb_sys_UG_Auth.FAuths ] = changed;//如权限有修改，更新最新的权限值
                }
        ...
        private void ShowAction(TreeNode node, DataRow row)
        {
            ...
            if (row != null) auth = int.Parse(row[tb_sys_UG_Auth.FAuths ].ToString());//用户拥有当前菜单的权限
        ...
        public void ShowUsers(ListBox list, DataTable users)
        {
            ...
            foreach (DataRow row in users.Rows)
            {
                ...
                ItemObject item = new ItemObject(row[tb_sys_User.FUserName].ToString() + factory, row);
                list.Items.Add(item);
            }
        }
        ...
        {
            IBridge_DataDict dataDict = BridgeFactory.CreateDataDictBridge(typeof(tb_sys_UserGroup));
            return dataDict.Update(save);
        }

        public int GetFormAuthority(string account, int moduleID, string menuName)
        {
            return _MyBridge.GetFormAuthority(account, moduleID);
        }
    }
