...
{/// <summary>
    ...
    public class DataConverter
    {
        ...
        /// <summary>
        /// C#数据类型转换成SQL SERVER数据类型
        /// </summary>
        /// <param name="sType">C#数据类型</param>
        /// <returns></returns>
        public static string CSharpDataTypeToSqlTypeString(string sType)
        {
            string strRet = "varchar";
            switch (sType)
            {
                case "System.Int16":
                case "System.Int32":
        ...
        public static string CSharpDataTypeToChTypeString(string sType)
        {
            string strRet = "文本";
            switch (sType)
            {
        ...
        public static string SqlTypeToString(SqlDbType sType)
        {
            string strRet = "varchar";
        ...
        /// <summary>
        /// 把一个行的数据更新到一个表中的一行,Key为主键
        /// </summary>
        /// <param name="dt"></param>
        /// <param name="dr"></param>
        /// <returns></returns>
        public static DataTable UpdateTableRowByRow(DataTable dt, DataRow dr,string Key)
        {

            bool b = false;
            for(int j=0;j<dt.Rows.Count;j++)
            {
                if(dt.Rows[j][Key].ToString()==dr[Key].ToString())
                {
                    for (int i = 0; i < dr.Table.Columns.Count; i++)
                    {
                        string colname = dr.Table.Columns[i].ColumnName;
                        if (dt.Columns.IndexOf(colname) >= 0)
                        {
                            if (dt.Rows[j][colname].ToString() != dr[colname].ToString())
                                dt.Rows[j][colname] = dr[colname];
                        }
                    }
                }
            }
        ...
        /// <returns></returns>
        public static DataTable TableColNameTUpper(DataTable table)
        {
            if(table!=null)
            {
        ...
        /// <returns></returns>
        public static System.Data.SqlClient.SqlParameter GetSqlPara(string ParaName, string DataType)
        {
        ...
        /// <param name="DataType"></param>
        /// <returns></returns>
        public static System.Data.OracleClient.OracleParameter GetOraclePara(string ParaName, string DataType)
        {
            switch (DataType.ToLower())
            {
                case "decimal":
