...
namespace SG.ORMTool
{
    ...
    public class GenerateSqlCmdBase : IGenerateSqlCommand
    {
        ...
        //Insert SQL命令
        protected IDbCommand _cmdInsert = null;

        //Update SQL命令
        protected IDbCommand _cmdUpdate = null;

        //Delete SQL命令
        protected IDbCommand _cmdDelete = null;
        ...
        /// <returns></returns>
        public IDbCommand GetInsertCommand(IDbTransaction tran)
        {
        ...
        /// <returns></returns>
        public IDbCommand GetUpdateCommand(IDbTransaction tran)
        {
        ...
        /// <param name="tran"></param>
        /// <returns></returns>
        public IDbCommand GetDeleteCommand(IDbTransaction tran)
        {
        ...
        /// <returns></returns>
        public string GernerateInsertSql(string tableName, string keyName, IList field,string DbType)
        {
            try
            {
                ...
                foreach (ColoumnProperty colProper in field)
                {
                    if (DbType == DbAcessTyp.SQLServer)
                        sb.Append("@" + colProper.ColumnName + ",");
                    else
                        sb.Append(":" + colProper.ColumnName + ",");
                }
        ...
        /// <param name="field">字段列表</param>
        /// <returns></returns>
        public string GernerateUpdateSql(string tableName, string keyName, IList field, string DbType)
        {
            try
            {
                StringBuilder sb = new StringBuilder();
                ...
                sb.Remove(sb.Length - 1, 1);
                sb.Append(" where 1=1 " + SplitKeyName(keyName,DbType));
                return sb.ToString().ToUpper();
            }
            catch
            { return string.Empty; }
        ...
        /// <param name="field">字段列表</param>
        /// <returns></returns>
        public string GernerateDeleteSql(string tableName, string keyName, IList field,string DbType)
        {
            try
            {
                StringBuilder sb = new StringBuilder();
                sb.Append("Delete " + tableName + " where 1=1" + SplitKeyName(keyName,DbType));
                return sb.ToString().ToUpper();
            }
            catch
            { return string.Empty; }
        ...
        /// <returns></returns>
        protected string SplitKeyName(string keyName, string DbType)
        {
